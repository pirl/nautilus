...
const applicationName = (options.wallet) ? 'Pirl-Nautilus-Wallet' : 'Mist';
    ...
    if (type === 'wallet') {
        if (options.walletSource === 'local') {
            ...
            bundle(`&& cd ../../meteor-dapp-wallet/app \
                && meteor-build-client ../../nautilus/dist_${type}/app/interface/wallet -p ""`);
        } else {
            ...
            bundle(`&& cd ../dist_${type} \
                && git clone --depth 1 https://github.com/pirl/meteor-dapp-wallet.git \
...
gulp.task('build-dist', (cb) => {
    const appPackageJson = _.extend({}, require('../package.json'), {  // eslint-disable-line global-require
        ...
        homepage: 'https://github.com/pirl/nautilus',
        build: {
